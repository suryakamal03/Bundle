    ...
    <DashboardLayout>
      {selectedTask && (
        <TaskDetailsModal
          ...
          onUpdate={async () => {
            // Refresh only the tasks data, not everything
            if (user) {
              const allUserTasks = await userTaskService.getUserTasks(user.uid)
              setTodoTasks(allUserTasks.filter(task => task.status === 'To Do'))
              setInReviewTasks(allUserTasks.filter(task => task.status === 'In Review'))
            }
