    ...
    if (tasks.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-600 dark:text-[#9a9a9a]">No tasks found</p>
        </div>
      )
    }
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge variant="info" className="text-xs bg-gray-100 dark:bg-[#1c1c1f] text-gray-700 dark:text-[#9a9a9a] border-gray-300 dark:border-[#26262a]">
                      {task.projectName || 'Unknown Project'}
                    </Badge>
                    <span className="text-xs text-gray-600 dark:text-[#9a9a9a]">
                      {task.status}
                    </span>
                  </div>
                </div>
                  ...
                  {activeTab === 'todo' && (
                    <button
                      ...
                      className={cn(
                        ...
                        hasReminder ? "bg-blue-500" : "bg-gray-300 dark:bg-[#26262a]"
                      )}
                      title={hasReminder ? "Reminder enabled" : "Reminder disabled"}
                    >
                      <div className={cn(
                        "w-5 h-5 rounded-full shadow-sm transition-transform absolute top-0.5",
                        hasReminder ? "translate-x-5 bg-white" : "translate-x-0.5 bg-gray-500 dark:bg-[#9a9a9a]"
                      )} />
                    </button>
                  )}
                </div>
  ...
  const renderIssues = () => {
    ...
    if (!githubUsername) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-600 dark:text-[#9a9a9a]">No GitHub username configured</p>
          <p className="text-xs text-gray-500 dark:text-[#9a9a9a] mt-1">Add your GitHub username in settings</p>
        </div>
      )
    }
    ...
    if (issues.length === 0) {
      return (
        <div className="text-center py-12">
          <AlertCircle className="w-12 h-12 text-gray-300 dark:text-[#26262a] mx-auto mb-3" />
          <p className="text-sm text-gray-600 dark:text-[#9a9a9a]">No open issues found</p>
        </div>
      )
    }
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant={issue.state === 'open' ? 'danger' : 'success'} className="text-xs bg-gray-100 dark:bg-[#1c1c1f] text-gray-700 dark:text-[#9a9a9a] border-gray-300 dark:border-[#26262a]">
                    {issue.state}
                  </Badge>
                  <span className="text-xs text-gray-600 dark:text-[#9a9a9a]">
            ...
            <div className="flex gap-2 mb-4 border-b border-gray-200 dark:border-[#26262a]">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium transition-colors ${
                  activeTab === 'todo'
                    ? 'text-gray-900 dark:text-[#eaeaea] border-b-2 border-blue-500 dark:border-white'
          ...
          <Card className="bg-white dark:bg-[#0f0f10] border-gray-200 dark:border-[#26262a]">
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-600 dark:text-[#9a9a9a]">No GitHub connected</p>
              </div>
            ) : activities.length === 0 ? (
              <div className="text-center py-8">
                <GitBranch className="w-10 h-10 text-gray-300 dark:text-[#26262a] mx-auto mb-2" />
                <p className="text-sm text-gray-600 dark:text-[#9a9a9a]">No recent activity</p>
              </div>
            ) : (
                ...
                {activities.slice(0, 8).map((activity) => {
                  ...
                  return (
                    <div
                      ...
                      className="flex gap-3 p-2 rounded-lg bg-white dark:bg-[#151517] border border-gray-200 dark:border-[#26262a] hover:bg-gray-50 dark:hover:bg-[#1c1c1f] transition-colors cursor-pointer"
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 bg-gray-100 dark:bg-[#1c1c1f] border border-gray-200 dark:border-[#26262a]`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-gray-600 dark:text-[#9a9a9a]">{activity.projectName}</span>
                          <span className="text-xs text-gray-600 dark:text-[#9a9a9a]">·</span>
                          <span className="text-xs text-gray-600 dark:text-[#9a9a9a]">{getRelativeTime(activity.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                  )
